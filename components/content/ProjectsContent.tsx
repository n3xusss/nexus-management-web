...
export default function Dashboard() {
  ...
  const fetchProjectsAndTasks = async () => {
    ...
    try {
      ...
      try {
        ...
        else if (memRes && Array.isArray(memRes.results)) memberData = memRes.results
      } catch (e) {
      ...
      // Map Projects
      const mappedProjects: Project[] = (Array.isArray(projData) ? projData : (projData.results || [])).map((p: any) => {
      ...
      // Map Tasks
      const mappedTasks: Task[] = (Array.isArray(taskData) ? taskData : (taskData.results || [])).map((t: any) => {
        ...
        return {
          ...
          due: new Date(t.due_date).toLocaleDateString('en-US', { day: 'numeric', month: 'long', year: 'numeric' }),
          progress: t.progress,
          status,
          description: t.task_description,
          started: new Date(t.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }),
          created: new Date(t.created_at).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }),
          lastUpdated: new Date(t.updated_at).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }),
        }
      })
      ...
      if (user?.role === 'admin' || user?.backendRole === 'mod') {
        try {
          ...
          const depts = Array.isArray(deptRes) ? deptRes : (deptRes.results || [])
          setAllDepts(depts)
        } catch (e) { }
      }
  ...
  const createTask = async () => {
    if (!newName.trim()) { alert('Task name is required'); return }
    if (!newMemberIds.length) { alert('At least one contributor is required'); return }
    if (!newDue) { alert('Deadline is required'); return }
  ...
  const createProject = async () => {
    if (!newProjName.trim()) { alert('Project name is required'); return }
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      if (editMemberIds.length > 0) payload.member_ids = editMemberIds.map(id => parseInt(id))
      ...
      <div className="max-w-[1400px] mx-auto px-6 py-8">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-400 mt-1">Track progress and manage tasks</p>
          </div>
          {(user?.role === 'admin' || user?.backendRole === 'mod') && (
            <button
              onClick={() => setShowCreateProjectModal(true)}
              className="flex items-center gap-1.5 bg-[#10b981] hover:bg-[#059669] px-6 py-3 rounded-lg text-sm font-semibold transition shadow-lg shadow-[#10b981]/20"
            >
        ...
        <div className={user?.role === 'admin' || user?.backendRole === 'mod' ? "block" : "grid lg:grid-cols-12 gap-6"}>

          {/* Projects */}
          <div className={user?.role === 'admin' || user?.backendRole === 'mod' ? "w-full" : "lg:col-span-8"}>
            <div className="bg-[#111113] rounded-2xl p-6 border border-[#1a1a1f]">
              <h2 className="text-xl font-semibold mb-5">Active Projects</h2>
              {projects.length === 0 ? (
                <p className="text-gray-500">No active projects found.</p>
              ) : (
                <div className={`grid gap-5 ${user?.role === 'admin' ? 'md:grid-cols-3' : 'md:grid-cols-2'}`}>
                  {projects.map((p, i) => (
                    <div key={i} className="bg-[#17171c] rounded-xl p-5 border border-[#222229]">
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          <h3 className="font-medium">{p.name}</h3>
                          <p className="text-xs text-[#8b5cf6] mt-0.5">{p.category}</p>
                        </div>
                        <span className="text-[#10b981] font-medium">{p.progress}%</span>
                      </div>
                      <div className="h-1.5 bg-[#1f1f24] rounded-full mb-3.5 overflow-hidden">
                        <div className="h-full bg-[#10b981]" style={{ width: `${p.progress}%` }} />
                      </div>
                ...
                <div className="flex gap-2 mb-5 flex-wrap">
                  {(['All', 'To Do', 'In Progress', 'Late'] as const).map(s => (
                    <button
                      ...
                      className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${activeFilter === s ? 'bg-[#4f46e5] text-white' : 'bg-[#1a1a1f] hover:bg-[#222229]'
                        }`}
                    >
                      {s}
                    </button>
                      ...
                      >
                        ...
                        <div className="mt-3 flex justify-between items-center text-xs">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`px-2.5 py-0.5 rounded-full text-[11px] ${task.status === 'In Progress' ? 'bg-[#6d28d9]/40 text-[#c084fc]' :
                              task.status === 'Late' ? 'bg-orange-900/60 text-orange-300' :
                                task.status === 'Overdue' ? 'bg-red-950/70 text-red-300' :
                  ...
                  <div>
                    ...
                    {members.length > 0 ? (
                      <select
                        multiple
                        value={newMemberIds}
                        onChange={e => setNewMemberIds(Array.from(e.target.selectedOptions, option => option.value))}
                      ...
                      <div className="bg-[#17171c] border border-[#222229] rounded-lg px-4 py-3 text-sm text-gray-500 flex items-center gap-2">
                        <AlertCircle size={16} />
                        {user ? `Task will be assigned to you. Backend enforces department permissions.` : 'No members available'}
                      </div>
                    )}
                    <p className="text-xs text-gray-500 mt-1">Hold Ctrl/Cmd to select multiple. Current: {newMemberIds.length}</p>
                  </div>
        ...
        showEditModal && editingTask && (
          <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
            <div className="bg-[#111113] rounded-2xl w-full max-w-md border border-[#1a1a1f] max-h-[90vh] overflow-y-auto">
                ...
                <div className="space-y-5">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-[#17171c] border border-[#222229] rounded-lg px-4 py-3 text-sm text-gray-300 focus:outline-none focus:border-[#6366f1]"
                    />
                    <p className="text-xs text-gray-500 mt-1">Leave empty to keep current deadline</p>
                  </div>
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-1.5">Add Contributors (Optional)</label>
                    {members.length > 0 ? (
                      <select
                        multiple
                        value={editMemberIds}
                        onChange={e => setEditMemberIds(Array.from(e.target.selectedOptions, option => option.value))}
                        className="w-full bg-[#17171c] border border-[#222229] rounded-lg px-4 py-3 text-sm text-gray-300 focus:outline-none focus:border-[#6366f1] h-24"
                      >
                        {members.map(m => (
                          <option key={m.id} value={m.id}>{m.username}</option>
                      ...
                      <div className="text-sm text-gray-500 italic">Contributor list unavailable</div>
                    )}
                  </div>
              ...
              <div className="flex justify-end gap-3 px-6 py-5 border-t border-[#1a1a1f]">
                <button onClick={() => setShowEditModal(false)} className="px-5 py-2.5 bg-[#1a1a1f] hover:bg-[#222229] rounded-lg text-sm">
                  Cancel
                </button>
                <button
                  onClick={saveEdit}
              ...
              <div className="p-6">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    ...
                    <p className="text-[#8b5cf6] mt-1 text-sm">{showDetail.project}</p>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => openEdit(showDetail)} className="p-2.5 bg-[#1a1a1f] hover:bg-[#222229] rounded-lg">
                      <Pencil size={18} />
                    </button>
                    <button onClick={() => deleteTask(showDetail.id)} className="p-2.5 bg-[#1a1a1f] hover:bg-[#222229] rounded-lg">
                ...
                <div className="grid md:grid-cols-5 gap-6">
                  <div className="md:col-span-3 space-y-6">
                    <div>
                      <h3 className="text-sm font-semibold mb-3">Task Description</h3>
                    ...
                    <div>
                      ...
                      <div className="bg-[#17171c] rounded-xl p-4 border border-[#222229]">
                        <div className="flex justify-between text-sm mb-2.5">
                          ...
                          <span className="text-[#10b981] font-medium">{showDetail.progress ?? 0}%</span>
                        </div>
                        <div className="h-2 bg-[#1f1f24] rounded-full overflow-hidden">
                          <div className="h-full bg-[#10b981]" style={{ width: `${showDetail.progress ?? 0}%` }} />
                        </div>
                      </div>
                    </div>

                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div><div className="text-gray-400">Started</div><div>{showDetail.started || '—'}</div></div>
                      <div><div className="text-gray-400">Due Date</div><div className="text-[#f97316]">{showDetail.due || 'Overdue'}</div></div>
                      <div><div className="text-gray-400">Created</div><div>{showDetail.created || '—'}</div></div>
                      <div><div className="text-gray-400">Last Updated</div><div>{showDetail.lastUpdated || '—'}</div></div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-[#111113] rounded-2xl w-full max-w-md border border-[#1a1a1f] max-h-[90vh] overflow-y-auto">
              <div className="p-6">
                ...
                <div className="space-y-5">
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-1.5">Assign Departments</label>
                    <div className="bg-[#17171c] border border-[#222229] rounded-lg p-3 space-y-2 max-h-40 overflow-y-auto">
                      {allDepts.map(dept => (
                        <label key={dept.id} className="flex items-center gap-3 cursor-pointer hover:bg-[#1a1a1f] p-1 rounded transition-colors">
                          <input
                            type="checkbox"
                            className="rounded border-[#222229] text-[#10b981] bg-[#111113] focus:ring-[#10b981]"
                            checked={assignedDeptIds.includes(dept.id.toString())}
                            onChange={e => {
                              if (e.target.checked) setAssignedDeptIds([...assignedDeptIds, dept.id.toString()])
                              else setAssignedDeptIds(assignedDeptIds.filter(id => id !== dept.id.toString()))
                            }}
                          />
                          <span className="text-sm text-gray-300">{dept.dept_name}</span>
                        </label>
                      ))}
                      {allDepts.length === 0 && <p className="text-gray-500 text-xs text-center py-2">No departments found</p>}
                    </div>
                  </div>
                </div>
              </div>
