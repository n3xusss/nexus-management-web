...
import ManagerDashboardContent from '../../components/content/ManagerDashboardContent'; // Add this import
...
const managerComponentMap = {
  dashboard: ManagerDashboardContent,
  projects: ProjectsContent,
  meetings: MeetingsContent,
  issues: IssuesContent,
  profile: ProfileContent,
...
// Helper function to get the appropriate component map based on role
const getComponentMap = (role: string) => {
  switch (role) {
    case 'admin':
      return adminComponentMap;
    case 'manager':
      return managerComponentMap;
    case 'member':
      return memberComponentMap;
    default:
      return memberComponentMap; // fallback
  }
};
...
export default function GlobalPage() {
  ...
  // Choose component map based on role using helper function
  const componentMap = getComponentMap(user.role);
  const activeSectionKey = activeSection as keyof typeof componentMap;
  const ActiveComponent = componentMap[activeSectionKey] || (() => {
    // Fallback based on role
    if (user.role === 'admin') return DashboardContent;
    if (user.role === 'manager') return ManagerDashboardContent;
  ...
  return (
    <div className="min-h-screen bg-[#2A2A2A] text-white flex overflow-hidden">
      {/* Fixed Sidebar */}
      ...
      <div className="flex-1 ml-64 flex flex-col min-h-screen">
        {/* Fixed Topbar */}
        <div className="fixed top-0 right-0 left-64 h-17 z-40 bg-gradient-to-r from-[#1e1e1e] to-[#2a2a2a] border-b border-[#3a3a3a] shadow-lg">
